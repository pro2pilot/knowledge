...
function findReceipt(raw) {
  ...
  // macOS exposes /var and /tmp through /private. A caller may retain the
  // logical spelling while secureStateStore returns the physical directory.
  // Normalize only those system aliases; user-controlled links are still
  // rejected by secureStateStore and loadReceipt.
  const parentIdentity = process.platform === 'win32'
    ? normalizeSystemAlias(path.dirname(absolute)).toLowerCase()
    : normalizeSystemAlias(path.dirname(absolute));
  const directoryIdentity = process.platform === 'win32'
    ? normalizeSystemAlias(directory).toLowerCase()
    : normalizeSystemAlias(directory);
...
function receiptTestEntry(loaded) {
  ...
  const executionPath = relativeContainedPath(stateRoot, loaded.path);
  if (!executionPath) {
    const error = new Error(
      'Verification execution record escaped its state root'
    );
    error.code = 'verification_execution_path_unsafe';
    throw error;
  }
  return {
