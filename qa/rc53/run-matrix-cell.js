...
// External RC53 compatibility harness. It never writes into the candidate ZIP
...
async function main() {
  ...
  write(path.join(out, 'environment.json'), {
    schema_version: 'rc53-os-node-environment.v1', os: osName, platform: process.platform, arch: process.arch, node: process.version, node_major: Number(nodeMajor), source_commit: sourceCommit,
  ...
  const routing = run('task-routing', process.execPath, [path.join(knowledge, 'tools', 'task-routing.js'), 'create', '--task', 'RC53 compatibility matrix', '--scope-path', '.knowledge/tools/flow.js', '--json'], { cwd: fixture, timeout: 300000 });
  ...
  if (upgradeEnabled) {
    // The conformance report can exceed macOS hosted-runner pipe thresholds.
    // Let the external replay tool write its JSON straight to the cell report
    // rather than serializing a large report through a child stdout pipe.
    const reportPath = path.join(out, 'upgrade-report.json');
    const runner = [
      "const fs=require('fs');",
      "const {smoke}=require(process.argv[1]);",
      "let report;",
      "try { report=smoke(process.argv[2],{previousArtifact:process.argv[3],keepFailed:true}); }",
      "catch (error) { report={schema_version:'conformance-install-smoke.v1',status:'fail',error:error.message}; }",
      "fs.writeFileSync(process.argv[4],JSON.stringify(report,null,2)+'\\n','utf8');",
      "if(report.status!=='pass') process.exitCode=2;"
    ].join('');
    const result = run('exact-upgrade', process.execPath, ['-e', runner, path.join(replay, 'tools', 'conformance-install-smoke.js'), candidate, baseline, reportPath], { cwd: externalCwd, timeout: 600000 });
    upgrade = parseJson(fs.readFileSync(reportPath, 'utf8'), 'exact-upgrade');
  ...
  write(path.join(out, 'commands.json'), { schema_version: 'rc53-matrix-commands.v1', commands });
  const result = { schema_version: 'rc53-os-node-cell.v1', status: 'pass', classification: 'candidate', candidate_sha256: candidateHash, tests: { shipped_self_tests: `${selfReport.passed}/26`, integration_runtimes: RUNTIMES.length, upgrade: upgrade.status }, command_count: commands.length, active_locks: 0 };
...
main().catch((error) => {
  try {
    write(path.join(out, 'commands.json'), { schema_version: 'rc53-matrix-commands.v1', commands });
    write(path.join(out, 'result.json'), { schema_version: 'rc53-os-node-cell.v1', status: 'fail', classification: 'candidate_or_harness_unclassified', error: { message: error.message, stack: error.stack || null }, command_count: commands.length });
  } catch (_) {}
