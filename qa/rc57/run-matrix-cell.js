...
async function main() {
  ...
  run('git-config-name', 'git', ['config', 'user.name', 'RC57 Matrix'], { cwd: fixture });
  fs.writeFileSync(path.join(fixture, 'README.fixture.md'), '# RC57 matrix fixture\n');
  ...
  if (selfTests.status !== 'pass' || selfTests.passed !== 26 || selfTests.failed !== 0) throw new Error('shipped self-tests are not 26/26');
  ...
  const routing = parseJson(run('task-routing', process.execPath, [
    path.join(knowledge, 'tools', 'task-routing.js'), 'create', '--task', 'RC57 OS/Node compatibility matrix',
  ...
  writeJson(path.join(out, 'commands.json'), { schema_version: 'rc57-matrix-commands.v1', commands });
  writeJson(path.join(out, 'environment.json'), {
    schema_version: 'rc57-os-node-environment.v1', os_label: osLabel,
  ...
  const result = {
    schema_version: 'rc57-os-node-cell.v1', status: 'pass', classification: 'candidate',
    candidate_sha256: candidateSha, tests: { shipped_self_tests: '26/26', integrations: '12/12', upgrade: upgrade.status },
...
main().catch((error) => {
  const result = {
    schema_version: 'rc57-os-node-cell.v1', status: 'fail', classification: 'unclassified',
    candidate_sha256: fs.existsSync(candidate) ? sha256File(candidate) : null,
    error: { message: error.message, stack: error.stack }
  };
  try { writeJson(path.join(out, 'result.json'), result); writeJson(path.join(out, 'commands.json'), { schema_version: 'rc57-matrix-commands.v1', commands }); } catch (_) {}
