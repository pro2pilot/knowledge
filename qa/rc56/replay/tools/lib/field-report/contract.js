...
function translationRequired(language, publicLanguage) {
  const source = normalizeLanguageTag(language || 'auto', { allowAuto: true });
  const target = normalizeLanguageTag(publicLanguage || 'en');
  return source === 'auto' || source.toLowerCase() !== target.toLowerCase();
}
