...
async function main() {
  ...
  write(path.join(out, 'environment.json'), {
    schema_version: 'rc54-os-node-environment.v1', os: osName, platform: process.platform, arch: process.arch, node: process.version, node_major: Number(nodeMajor), source_commit: sourceCommit,
  ...
  write(path.join(out, 'commands.json'), { schema_version: 'rc54-matrix-commands.v1', commands });
  const result = { schema_version: 'rc54-os-node-cell.v1', status: 'pass', classification: 'candidate', candidate_sha256: candidateHash, tests: { shipped_self_tests: `${selfReport.passed}/26`, integration_runtimes: RUNTIMES.length, upgrade: upgrade.status }, command_count: commands.length, active_locks: 0 };
...
main().catch((error) => {
  try {
    write(path.join(out, 'commands.json'), { schema_version: 'rc54-matrix-commands.v1', commands });
    write(path.join(out, 'result.json'), { schema_version: 'rc54-os-node-cell.v1', status: 'fail', classification: 'candidate_or_harness_unclassified', error: { message: error.message, stack: error.stack || null }, command_count: commands.length });
  } catch (_) {}
