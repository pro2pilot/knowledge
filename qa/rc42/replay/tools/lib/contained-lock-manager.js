...
function inspectCurrent(policy, paths) {
  ...
  try { read = readStrictOwner(policy.rootPath, paths.lockDir, { lockName: policy.lockName, purpose: policy.purpose }); }
    ...
    if (error.code === 'lock_owner_invalid' && error.reason === 'missing' && Date.now() - stat.mtimeMs < LOCK_POLICY.owner_initialization_grace_ms) {
      return { status: 'initializing', age_ms: Math.max(0, Date.now() - stat.mtimeMs) };
    }
    throw error;
  }
  const ageMs = Math.max(0, Date.now() - stat.mtimeMs);
