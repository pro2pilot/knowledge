...
function run(argv = process.argv.slice(2)) {
  ...
  fixture('parallel-agents', (repoRoot) => {
    ...
    const concurrentRunner = [
      ...
      "  const child = spawn(process.execPath, ['.knowledge/tools/install-agent-integrations.js', '--runtime', runtime, '--no-package-scripts', '--no-install-check'], { stdio: 'ignore' });",
      "  child.on('error', (error) => resolve([runtime, { error: error.code || error.message }]));",
      "  child.on('exit', (code) => resolve([runtime, code]));",
    ...
    const agents = read(path.join(repoRoot, 'AGENTS.md'));
    const attributes = read(path.join(repoRoot, '.gitattributes'));
    check('parallel-agents-compatible-install', Object.keys(exits).length === agentsRuntimes.length && Object.values(exits).every((code) => code === 0) && agents.split(managedStart).length - 1 === 1 && agents.includes('USER PARALLEL PRE') && agents.includes('USER PARALLEL POST') && expectedAgentAttributes.every((item) => attributes.includes(item)) && fs.existsSync(path.join(repoRoot, '.devin', 'rules', 'knowledge.rules')), { exits, launcher_exit: parallel.status });
  });
