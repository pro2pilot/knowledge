...
Этот файл превращает приложенный текст про team mode в проектную спецификацию для `.knowledge 3.2.4`. Режим должен быть экспериментальным, но рабочим: несколько агентов работают в разных Git worktrees/branches, имеют общий team registry, отдельные workspace states, стабильные agent IDs, централизованные locks/events/status и безопасную merge-модель через Git.
