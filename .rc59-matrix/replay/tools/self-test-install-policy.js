...
function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    record(results, 'existing update preserves project knowledge', () => {
      ...
      if (process.platform === 'darwin' && /^\/private\/var(?:\/|$)/.test(applyJson.backup_path || '')) {
        const aliasedApplyReport = JSON.parse(JSON.stringify(applyJson));
        aliasedApplyReport.backup_path = aliasedApplyReport.backup_path.replace(/^\/private\/var(?=\/|$)/, '/var');
        writeJson(path.join(repo, '.knowledge', 'maintenance', 'update_system_files_report.json'), aliasedApplyReport);
        const aliasedVerify = runNode([
        ...
        const aliasedVerifyJson = parseJsonResult(aliasedVerify);
        assert(
          aliasedVerify.exit === 0 && aliasedVerifyJson.status === 'ok',
          'verify-upgrade rejected the documented /var alias of a retained backup path.',
          aliasedVerifyJson
        );
      }
