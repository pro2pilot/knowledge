...
function contained(candidate, root) {
  const resolved = normalizeSystemAlias(candidate);
  const base = normalizeSystemAlias(root);
  return resolved === base || resolved.startsWith(`${base}${path.sep}`);
}
function pathIdentity(value) {
  const resolved = normalizeSystemAlias(value);
...
function appendReceiptExecutionGuards(
  ...
  for (const test of receipt.tests_run || []) {
    ...
    const expectedPath = relativeContainedPath(stateRoot, execution.path);
    if (
...
function applyVerificationReceipt(receipt, request = {}) {
  ...
  const relativeReceipt = relativeContainedPath(stateRoot, savedReceipt.path);
  if (!relativeReceipt) {
    return {
      ...base,
      status: 'rejected',
