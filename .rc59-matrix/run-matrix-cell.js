...
async function main() {
  ...
  run('git-config-name', 'git', ['config', 'user.name', 'RC58 Matrix'], { cwd: fixture });
  fs.writeFileSync(path.join(fixture, 'README.fixture.md'), '# RC58 matrix fixture\n');
  ...
  const routing = parseJson(run('task-routing', process.execPath, [
    path.join(knowledge, 'tools', 'task-routing.js'), 'create', '--task', 'RC58 OS/Node compatibility matrix',
  ...
  writeJson(path.join(out, 'commands.json'), { schema_version: 'rc58-matrix-commands.v1', commands });
  writeJson(path.join(out, 'environment.json'), {
    schema_version: 'rc58-os-node-environment.v1', os_label: osLabel,
  ...
  const result = {
    schema_version: 'rc58-os-node-cell.v1', status: 'pass', classification: 'candidate',
...
main().catch((error) => {
  const result = {
    schema_version: 'rc58-os-node-cell.v1', status: 'fail', classification: 'unclassified',
    candidate_sha256: fs.existsSync(candidate) ? sha256File(candidate) : null,
    error: { message: error.message, stack: error.stack }
  };
  try { writeJson(path.join(out, 'result.json'), result); writeJson(path.join(out, 'commands.json'), { schema_version: 'rc58-matrix-commands.v1', commands }); } catch (_) {}
