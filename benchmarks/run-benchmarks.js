...
function gate00Result(gate) {
  if (gate?.status === 'pass') {
    return passResult(
      {
        release_gate_passed: 1,
        p0_commands: (gate.steps || []).length,
        clean_install_steps: (gate.clean_install?.steps || []).length
      },
  ...
  return {
    ...
    metrics: { release_gate_passed: 0 },
    evidence: ['maintenance/release-gate-report.json'],
    limitations: ['release gate report is missing or not passed']
  };
}
...
function runSuiteOnce(slug) {
  ...
  if (slug === 'gate-00') {
    return gate00Result(readJson(path.join(root, 'maintenance', 'release-gate-report.json'), {}));
  }
...
if (require.main === module) {
  try { main(); }
    ...
    const output = { schema_version: SCHEMA_VERSION, status: 'failed', error: sanitizeText(error.message) };
    if (flags.json) console.log(JSON.stringify(output, null, 2));
...
module.exports = { main, runSuiteOnce, aggregateSuite, gate00Result };
