...
function answerTemplate(manifest, questions) {
  return {
    ...
    instructions: [
      ...
      'Answers may use any source language, but the publication-ready draft is always English.',
      'Use generalized descriptions instead of internal workspace, department, or client names.'
    ],
...
function migrateManifestPublicationContract(context, manifest) {
  const finalState = ['published', 'cancelled'].includes(manifest.status);
  let targetLanguage = PUBLIC_LANGUAGE;
  let languageInvalid = false;
  try {
    targetLanguage = normalizePublicLanguage(manifest.public_language || PUBLIC_LANGUAGE);
  } catch {
    languageInvalid = true;
    targetLanguage = PUBLIC_LANGUAGE;
  }
  const contractChanged = manifest.contract_version !== CONTRACT_VERSION;
  const languageChanged = manifest.public_language !== targetLanguage || languageInvalid;
  if (!contractChanged && !languageChanged) return manifest;
  if (finalState) return manifest;
  if (['publishing', 'reconciliation_required'].includes(manifest.status)) {
    fail(
      'The Field Report publication contract changed while publication requires reconciliation.',
      2,
      'field_report_contract_reconciliation_required'
    );
  }

  const reportPaths = state.paths(context, manifest.report_id);
  const original = readJson(reportPaths.answers_original, {});
  const sourceLanguage = normalizeLanguageTag(manifest.language || 'auto', { allowAuto: true });
  manifest.language = sourceLanguage;
  manifest.public_language = targetLanguage;
  manifest.contract_version = CONTRACT_VERSION;
  resetTranslation(manifest, reportPaths, original, 'publication_contract_changed');
  state.invalidateApproval(manifest);

  const complete = Object.keys(original).length > 0 && missingQuestions(original).length === 0;
  const needed = translationRequired(sourceLanguage, targetLanguage);
  if (needed && complete) {
    if (manifest.status === 'approved') state.transition(manifest, 'draft_ready');
    if (manifest.status === 'preview_ready' || manifest.status === 'publish_failed') {
      state.transition(manifest, 'draft_ready');
    }
    if (['draft_ready', 'redaction_required', 'translation_ready', 'needs_user_input'].includes(manifest.status)) {
      state.transition(manifest, 'translation_required');
    }
  } else if (!needed && ['translation_required', 'translation_ready'].includes(manifest.status)) {
    state.transition(manifest, 'needs_user_input');
  }
  manifest.warnings = [...new Set([
    ...(manifest.warnings || []),
    'Field Report public output is fixed to English; the draft was migrated to the current publication contract.'
  ])];
  state.save(context, manifest);
  return manifest;
}
...
function resetTranslation(manifest, reportPaths, originalAnswers, reason = 'original_answers_changed') {
...
function translationExport(context, flags, manifest) {
  ...
  // A translation export is an outbound artifact. Build it from the same
  // allowlist/redaction boundary used by publication rather than exposing the
  // immutable internal answers verbatim. Deterministic generalization and
  // medium-severity redaction are allowed; unresolved high/critical findings
  // remain fail-closed.
  const exportRedaction = redactAnswers(original, manifest.anonymized);
  if (exportRedaction.report.status === 'blocked') {
    fail(
  ...
  const safeOriginal = exportRedaction.answers;
  const exportedAnswersHash = canonicalHash(safeOriginal);
  const exportBody = JSON.stringify(safeOriginal);
  const exportPrivacy = scanPublication({ body: exportBody }, manifest.anonymized);
  if (exportPrivacy.report.status === 'blocked' || exportPrivacy.body !== exportBody) {
    fail(
      'Translation export could not be made privacy-stable.',
      2,
      'translation_export_privacy_unstable'
    );
  }
  ...
  manifest.translation.exported_answers_hash = exportedAnswersHash;
  ...
  writeJsonAtomic(reportPaths.translation_provenance, {
    ...
    exported_answers_hash: exportedAnswersHash,
    exported_at: new Date().toISOString(),
    export_privacy: {
      status: exportRedaction.report.status,
      redactions: exportRedaction.report.redactions,
      unresolved_findings: exportRedaction.report.unresolved_findings
    },
    rules: [
  ...
  return {
    ...
    original_hash: originalHash,
    exported_answers_hash: exportedAnswersHash,
    original_answers: safeOriginal,
    export_privacy: {
      status: exportRedaction.report.status,
      redactions: exportRedaction.report.redactions.length,
      unresolved_findings: exportRedaction.report.unresolved_findings.length
    },
    expected_response: {
      original_hash: originalHash,
      exported_answers_hash: exportedAnswersHash,
...
function translationIngest(context, flags, manifest, injected) {
  ...
  const exported = readJson(reportPaths.translation_provenance, null);
  const exportRedaction = redactAnswers(original, manifest.anonymized);
  if (exportRedaction.report.status === 'blocked') {
    fail('The current translation source cannot pass the privacy boundary.', 2, 'translation_export_privacy_blocked');
  }
  const expectedOriginalHash = canonicalHash(originalValidation.answers);
  const expectedExportedHash = canonicalHash(exportRedaction.answers);
  if (
    !exported ||
    exported.schema_version !== 'knowledge-field-report-translation.v1' ||
    exported.report_id !== manifest.report_id ||
    exported.status !== 'translation_required' ||
    exported.source_language !== manifest.language ||
    exported.target_language !== manifest.public_language ||
    exported.original_hash !== expectedOriginalHash ||
    exported.exported_answers_hash !== expectedExportedHash
  ) {
    fail(
      'Export the current privacy-stable original answers before ingesting a translation.',
      2,
      'translation_export_required'
    );
  }
  ...
  if (!input.exported_answers_hash || input.exported_answers_hash !== expectedExportedHash) {
    fail(
      'Translation exported_answers_hash does not match the exact privacy-stable source export.',
      2,
      'translation_export_stale'
    );
  }
  ...
  writeJsonAtomic(reportPaths.translation_provenance, {
    ...
    exported_answers_hash: expectedExportedHash,
...
function translationApprove(context, flags, manifest, injected = {}) {
  ...
  const currentExport = redactAnswers(original, manifest.anonymized);
  if (currentExport.report.status === 'blocked') {
    fail('The current translation source cannot pass the privacy boundary.', 2, 'translation_export_privacy_blocked');
  }
  const exportedAnswersHash = canonicalHash(currentExport.answers);
  if (originalHash !== review.original_hash || translatedHash !== review.translated_hash ||
      originalHash !== provenance.original_hash ||
      exportedAnswersHash !== provenance.exported_answers_hash ||
      exportedAnswersHash !== review.exported_answers_hash ||
      exportedAnswersHash !== manifest.translation.exported_answers_hash ||
...
function run(argv = process.argv.slice(2), injected = {}) {
  ...
  return state.lock(context, manifest.report_id, () => {
    ...
    manifest = migrateManifestPublicationContract(context, manifest);
...
module.exports = {
  ...
  __test: {
    ...
    migrateManifestPublicationContract,
    openCommand,
    publicationMaterial
  }
};
