...
async function main() {
  ...
  try {
    ...
    assert(stateRes.status === 200 && stateRes.json?.state?.product?.version === '3.2.3', 'api state did not return product 3.2.3');
    ...
    const result = { schema_version: '3.2.3', status: 'pass', checks: ['one-file launcher starts', 'click launcher files exist', 'trust gate asset exists', 'launcher helper exists', 'session token required', 'canonical nav renders', 'shared Inspector renderer renders'] };
