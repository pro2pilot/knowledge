...
function contained(candidate, root) {
  const absolute = normalizeSystemAlias(candidate);
  const base = normalizeSystemAlias(root);
  return absolute === base || absolute.startsWith(`${base}${path.sep}`);
}

function relativeContainedPath(root, candidate) {
  const base = normalizeSystemAlias(root);
  const absolute = normalizeSystemAlias(candidate);
  if (!contained(absolute, base)) return null;
...
function runVerificationTests({
  ...
  return tests.map((test) => {
    ...
    const relativePath = relativeContainedPath(stateRoot, saved.path);
    if (!relativePath) {
      throw contentStoreError(
        'verification_execution_path_unsafe',
...
function validateReceipt(raw, options = {}) {
  ...
  for (const test of tests) {
    ...
    const expectedPath = relativeContainedPath(options.stateRoot, loaded.path);
