...
// macOS exposes /var and /tmp as documented system aliases below /private.
// Treat only an exact system-alias resolution as physical. Any user-created
// symlink/junction remains fail-closed.
function canonicalSystemAlias(root, real) {
  if (process.platform !== 'darwin') return null;
  for (const [alias, physical] of [['/var', '/private/var'], ['/tmp', '/private/tmp']]) {
    if ((root === alias || root.startsWith(`${alias}/`)) &&
        (real === physical || real.startsWith(`${physical}/`))) {
      const expected = `${physical}${root.slice(alias.length)}`;
      if (pathIdentity(real) === pathIdentity(expected)) return real;
    }
  }
  return null;
}

function normalizeSystemAlias(value) {
  const resolved = path.resolve(value);
  if (process.platform !== 'darwin') return resolved;
  for (const [alias, physical] of [['/var', '/private/var'], ['/tmp', '/private/tmp']]) {
    if (resolved === alias || resolved.startsWith(`${alias}/`)) {
      return `${physical}${resolved.slice(alias.length)}`;
    }
  }
  return resolved;
}
...
function containedPath(rootPath, candidatePath) {
  const root = normalizeSystemAlias(rootPath);
  const candidate = normalizeSystemAlias(candidatePath);
...
function assertSafeContainmentRoot(rootPath) {
  ...
  const systemAlias = canonicalSystemAlias(root, real);
  if (!stat.isDirectory() || (stat.isSymbolicLink() && !systemAlias)) {
    throw containmentError(
      `Containment root is not a physical directory: ${root}`
    );
  }
  if (!systemAlias && pathIdentity(real) !== pathIdentity(root)) {
    throw containmentError(
      `Containment root resolves through a reparse path: ${root}`
    );
  }
  return systemAlias || root;
}
...
function ensureContainedDir(rootPath, dirPath) {
  const root = assertSafeContainmentRoot(rootPath);
  const directory = normalizeSystemAlias(dirPath);
