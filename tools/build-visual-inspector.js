...
function renderPaidPreview(data) {
  ...
  const billing = manifest.billing_dimensions.slice(0, 8);
  const doNotCharge = manifest.do_not_charge_for.slice(0, 8);
  const policy = manifest.pricing_policy || {};
  const capabilityHtml = capabilities.length
    ? `<div class="paid-grid">${capabilities.map((item) => `<button type="button" class="paid-action" disabled title="${esc(item.paid_value || 'Paid Inspector extension point')}">${esc(item.label || item.id)}<span>${esc(item.free_behavior || 'disabled_preview')}</span></button>`).join('')}</div>`
  ...
  const signalHtml = signalRows.length
    ? `<div class="signal-grid">${signalRows.map((signal) => `<div class="signal-card ${signal.active ? 'active' : ''}"><strong>${esc(signal.label || signal.id)}</strong><span>${esc(signal.active ? 'Signal active' : 'Available action')}</span><p>${esc(signal.message || '')}</p><button type="button" class="paid-action" disabled>${esc(signal.action_label || 'Paid action')}<span>Paid preview</span></button></div>`).join('')}</div>`
    : '<p class="sub">No conversion signals configured.</p>';
  const billingHtml = billing.length
    ? `<table class="kv"><tbody>${billing.map((item) => `<tr><th>${esc(item.label || item.id)}</th><td>${esc(item.paid_reason || '')}</td></tr>`).join('')}</tbody></table>`
    : '<p class="sub">No billing dimensions configured.</p>';
  return `<div class="paid-meta"><strong>${esc(manifest.packaging_copy?.paid?.headline || 'Paid Inspector surface')}</strong><span>Prices in free core: ${esc(policy.prices_in_free_core)}</span><span>Feature-plan bindings in free core: ${esc(policy.feature_plan_bindings_in_free_core)}</span></div>${capabilityHtml}<h3>Contextual Conversion Signals</h3>${signalHtml}<h3>Billing Boundaries</h3>${billingHtml}<h3>Do Not Charge For</h3><p class="sub">${esc(doNotCharge.join('; ') || 'No free boundary configured.')}</p><h3>Usage Billing Policy</h3><p class="sub">${esc(manifest.usage_billing_policy?.initial_policy || 'No usage billing policy configured.')}</p><p class="sub">Paid implementation is intentionally outside the free package; this static Inspector only exposes manifest-driven disabled actions, conversion signals, and local truth.</p>`;
}
...
function renderTabbed(data, options = {}) {
  ...
  const tabs = [
    ...
    ['Pro Preview', `<div class="card"><h2>Coming Soon</h2><p class="sub">Pro unlocks deeper governance inside this same Inspector. Free .knowledge stays local-first and fully usable. No prices are shown inside the free Inspector.</p>${renderPaidPreview(data)}</div>`]
  ];
