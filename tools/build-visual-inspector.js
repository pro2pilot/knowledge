...
} = require('./lib/paid-inspector-model');
...
function collect() {
  ...
  const paidManifest = normalizePaidManifest(safeJson('docs/product/paid-feature-manifest.json', {}));
  let team = null;
  let lockOwner = null;
  if (context.teamRoot) {
...
function renderQuickActions(options = {}) {
  ...
  const actions = [
    ...
    ['Export Debug Bundle', 'node .knowledge/tools/export-debug-bundle.js --json'],
    ...
    ['Export Pro Snapshot', 'node .knowledge/tools/export-pro-snapshot.js --json']
  ];
  ...
  const liveButtons = liveActions.map((action) => {
    ...
    const locked = action.risk === 'pro_only' ? ' locked' : '';
...
function renderPaidPreview(data) {
  const manifest = normalizePaidManifest(data.paidManifest || {});
  const metrics = paidSurfaceMetrics(data);
  const capabilities = manifest.capabilities.slice(0, 24);
  const signals = manifest.conversion_signals.map((signal) => ({
    ...signal,
    active: evaluateConversionSignal(signal, metrics)
  }));
  const activeSignals = signals.filter((signal) => signal.active);
  const inactiveSignals = signals.filter((signal) => !signal.active).slice(0, 6);
  const signalRows = [...activeSignals, ...inactiveSignals];
  const previewButtonStyle = 'border:1px solid #344852;background:#121f28;color:#dce8df;border-radius:6px;padding:10px 12px;text-align:left;min-height:48px;opacity:.82';
  const previewButtonMetaStyle = 'display:block;color:#aebbb3;font-size:11px;margin-top:3px';
  const capabilityHtml = capabilities.length
    ? `<div class="paid-grid">${capabilities.map((item) => `<button type="button" class="paid-action" aria-disabled="true" tabindex="-1" title="Inspector Pro preview" style="${previewButtonStyle}">${esc(proPreviewText(item.label || item.id))}<br><span style="${previewButtonMetaStyle}">${esc(proPreviewText(item.free_behavior || 'waitlist_preview'))}</span></button>`).join('')}</div>`
    : emptyState('No paid capabilities configured', 'Add capabilities to docs/product/paid-feature-manifest.json.', 'node .knowledge/tools/validate-paid-manifest.js');
  const signalHtml = signalRows.length
    ? `<div class="signal-grid">${signalRows.map((signal) => `<div class="signal-card ${signal.active ? 'active' : ''}"><strong>${esc(proPreviewText(signal.label || signal.id))}</strong><span>${esc(signal.active ? 'Signal active' : 'Available action')}</span><p>${esc(proPreviewText(signal.message || ''))}</p><button type="button" class="paid-action" aria-disabled="true" tabindex="-1" style="${previewButtonStyle};min-height:0;padding:7px 10px">${esc(proPreviewText(signal.action_label || 'Join waitlist'))}<span style="${previewButtonMetaStyle}">Waitlist preview</span></button></div>`).join('')}</div>`
    : '<p class="sub">No Pro signals configured.</p>';
  return `<div class="pro-preview-intro"><div><strong>Inspector Pro Preview</strong><span>Advanced team review, governance, and audit workflows will connect to the same local-first trust layer.</span></div><a class="pro-waitlist-button" style="letter-spacing:0" href="https://pro2pilot.com/inspector/" target="_blank" rel="noopener">Inspector Pro Waitlist</a></div>${capabilityHtml}<h3>Inspector Pro Signals</h3>${signalHtml}`;
}
...
function proPreviewText(value) {
  return String(value || '')
    ...
    .replace(/readonly_local_status/gi, 'Local status')
    .replace(/markdown_preview/gi, 'Preview')
    .replace(/local_queue_only/gi, 'Local queue')
    .replace(/advisory_preview/gi, 'Preview')
    .replace(/local_history_only/gi, 'Local history')
    .replace(/not_available/gi, 'Preview pending');
}
...
<section class="grid two"><div class="card"><h2>PR Summary Preview</h2>${renderPrPreview(data)}</div><div class="card"><h2>Paid Inspector Surface</h2>${renderPaidPreview(data)}</div></section>
</main>
...
function renderTabbed(data, options = {}) {
  ...
  const tabs = [
    ...
    ['Pro Preview', `<div class="card"><h2>Coming Soon</h2><p class="sub">Inspector Pro is planned for teams that need deeper collaboration and governance. Free .knowledge remains local-first and fully usable.</p>${renderPaidPreview(data)}</div>`]
  ];
...
function build(options = {}) {
  ...
  const status = {
    ...
    features: [
      ...
      'paid_extension_points',
      'config_driven_paid_manifest',
