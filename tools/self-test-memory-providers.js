...
function testPythonDiscovery(root) {
  ...
  const containedLockTimeout = Object.assign(new Error('Lock "memory-provider" is held by pid 123 on host "test".'), { code: 'lock_timeout' });
  assert(mem0Main.__test.isContainedLockBusyError(containedLockTimeout) === true, 'contained lock timeout must classify as qdrant lock busy');
  assert(mem0Main.__test.isContainedLockBusyError(new Error('unrelated runtime failure')) === false, 'unrelated runtime errors must not classify as qdrant lock busy');
