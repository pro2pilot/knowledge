...
function main() {
  ...
  try {
    check('00 macOS system temporary-directory aliases preserve safe verification cwd', () => {
      if (process.platform !== 'darwin') return;
      const fixtureRoot = path.join(root, 'darwin-system-alias-verification');
      const context = fixtureContext(fixtureRoot);
      writeText(path.join(fixtureRoot, 'src', 'probe.js'), 'module.exports = true;\n');
      writeText(path.join(fixtureRoot, 'tests', 'probe.js'), 'process.exit(0);\n');
      const executions = runVerificationTests({
        stateRoot: context.stateRoot,
        repoRoot: fixtureRoot,
        taskId: 'darwin-system-alias-verification',
        sessionId: 'darwin-system-alias-session',
        tests: [{ argv: ['node', 'tests/probe.js'] }],
        sourceFiles: [{ path: 'src/probe.js' }],
        checkedBy: 'repair-test'
      });
      assert(
        executions.length === 1 &&
          executions[0].record.status === 'pass' &&
          executions[0].record.cwd === '.' &&
          /^maintenance\/verification_executions\/[a-f0-9]{64}\.json$/.test(executions[0].relative_path),
        'macOS system temporary-directory alias rejected the repository verification cwd'
      );
    });
    ...
    check('64i approved search producer rejects output and input junction escapes', () => {
      ...
      assert(
        ...
        aliasCodes.every((code) => [
          ...
          'verification_execution_invalid',
    ...
    // saveReceipt returns a physical path. On macOS the fixture root can
    // retain the logical /var spelling, so calculate the fixture-relative
    // receipt name from the same physical identity before copying it.
    const applyReceiptRelative = path.relative(
      fs.realpathSync(applyContext.stateRoot),
      fs.realpathSync(applyReceiptFile)
    );
    assert(
      applyReceiptRelative &&
      !applyReceiptRelative.startsWith(`..${path.sep}`) &&
      applyReceiptRelative !== '..' &&
      !path.isAbsolute(applyReceiptRelative),
      'apply receipt escaped its verification receipt store'
    );
    let applyResult;
