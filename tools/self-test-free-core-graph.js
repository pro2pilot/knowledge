...
const systemVersion = JSON.parse(fs.readFileSync(path.join(root, 'package.json'), 'utf8')).version || '3.3.0';
...
function ensureMinimumGraphFixture() {
  const registryPath = path.join(root, 'modules', 'module_registry.json');
  let registry = null;
  try { registry = JSON.parse(fs.readFileSync(registryPath, 'utf8')); } catch { registry = null; }
  if (Array.isArray(registry?.modules) && registry.modules.length > 0) return false;
  fs.mkdirSync(path.dirname(registryPath), { recursive: true });
  fs.writeFileSync(registryPath, JSON.stringify({
    modules: [{ module_id: 'root', path: '.', purpose: 'Self-contained free-core graph fixture', key_files: ['README.md'], trust_status: 'routing_trusted' }]
  }, null, 2) + '\n');
  const targetReadme = path.join(root, '..', 'README.md');
  if (!fs.existsSync(targetReadme)) fs.writeFileSync(targetReadme, '# Graph self-test fixture\n');
  return true;
}

function main() {
  const fixtureCreated = ensureMinimumGraphFixture();
