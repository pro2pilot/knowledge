...
function main(){ensureDir(path.join(knowledgeRoot,'evaluation','results')); const results=checks.map(([name,rel])=>({name,...run(rel)})); const score=Math.round(results.filter(r=>r.status==='pass').length/results.length*100); const report={schema_version:'3.1.9',generated_at:new Date().toISOString(),score,status:score>=90?'release_candidate':score>=75?'usable_with_warnings':'needs_repair',results}; writeJsonAtomic(path.join(knowledgeRoot,'evaluation','results','latest.json'),report); console.log(JSON.stringify(report,null,2));}
